...
# Django 5.1+ 移除了 force_text，Python 3 直接使用 str
force_text = str
...
DATETIME_REGEX = re.compile(r'^(?P<year>\d{4})-(?P<month>\d{2})-(?P<day>\d{2})T(?P<hour>\d{2}):(?P<minute>\d{2}):(?P<second>\d{2})(\.\d{3,6}Z?)?$')
...
class WhooshSearchBackend(BaseSearchBackend):
    ...
    def build_schema(self, fields):
        ...
        for field_name, field_class in fields.items():
            if field_class.is_multivalued:
                ...
                analyzer = ChineseAnalyzer() if ChineseAnalyzer else None
                schema_fields[field_class.index_fieldname] = TEXT(stored=True, analyzer=analyzer, field_boost=field_class.boost, sortable=True)
    ...
    def _to_python(self, value):
        ...
        try:
            ...
            if isinstance(converted_value, (list, tuple, set, dict, int, float, complex)):
